...
- 消去: **🩹消しゴム**ボタンでモードON→1本指でなぞると塗り/駅状態を部分消去(`eraseMode`/`eraseAt`/`pickErase`)。
  消しゴム中は1本指=消去なのでパンは2本指。**全消去**ボタンは押すと`confirm()`で確認してから全クリア。
  ※`pickErase`は未訪問駅や未塗装を飛ばして「消せる対象(塗り済み区間/状態付き駅)」だけを近傍探索。
  ※区間描画クラスは`segClass(s)`で新幹線(shink)/塗り(on)を保持(タップ時にshinkが消えないように)。
...
- 温泉/観光は内蔵サンプル(POIS)。
- 境界線: `setBorders`が都道府県ポリゴンの辺を「隣接県と共有する辺=県境(`kenkyo`, `.border.ken`で薄い#ccd4db)」と
  「1県だけの辺=海岸線/外周(`borders`, `.border` #8fa0b0のまま)」に分離(辺を4桁で照合し出現回数で判定、
  連続する同種辺は1パスにまとめる)。県境を薄くして路線と区別しやすく(外周は従来通り)。
