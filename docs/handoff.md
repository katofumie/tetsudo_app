...
- 色/見た目（2026-06 改善）:
  ...
  - **新幹線は線路風(枕木)**: 名前に「新幹線」を含む路線(`isShink`)は太いレール(`.seg.shink`)＋**線に垂直な短い線
    (枕木)を等間隔で描画**(`renderShinkTies()`→`gTie`グループ, `.tie2`)。枕木は画面px基準の間隔/長さで、拡大時
    (s≤0.02)のみ描く(全国表示は太線のまま=重さ対策)。色は本線と同じ(未塗装=くすみ色/塗り済み=緑)。
    パン/ズーム時は`scheduleStations()`で再生成。※破線オーバーレイ方式は「ただの破線」で枕木に見えず廃止(2026-06)。
