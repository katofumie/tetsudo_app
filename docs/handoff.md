...
- 色/見た目（2026-06 改善）:
  ...
  - **路線名を線路に沿って表示**: `renderLineLabels()`が拡大時(s≤0.012, 主要駅名と同じころ)に、画面内の各路線
    から画面中心に近い区間を1つ選び、その向きに回転した**文字のみ**(背景なし)で名前を描画。線路に重ねないよう
    垂直方向に少し上へオフセットし、控えめな小フォント＋薄いハロー。重なりは貪欲法で間引き(中心に近い路線優先)。
    スマホでもタップ不要で読める(従来のmouseoverツールチップは残置)。`gLine`はz順で最前面へ。
    パン/ズーム時は`scheduleStations()`内で`renderStations()`と一緒に再描画。
    ※ピル(白バッジ)版は駅名や線路を隠すため廃止(ユーザー指摘 2026-06)。
